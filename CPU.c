...
void CPUPrintNextPcSource(CPU_Ptr cpuPtr);
...
void CPUExecuteInstruction(CPU_Ptr cpuPtr, InstructionFieldsPtr instructionFieldsPtr)
{
    if (cpuPtr->controller.uncondBranch == UNCOND_BRANCH_FLAG_OFF)
    {
        ...
        else if (cpuPtr->controller.condBranch == COND_BRANCH_FLAG_ON)
        {
...
void CPUPrintNextPcSource(CPU_Ptr cpuPtr)
{
    printf("CPU Next Program Counter (PC) Source: %u \n\n", cpuPtr->pcSourceSelector);
}
